...
def document_structure(ftex_in):
    ...
    while i < nln:
        ...
        if re.match(r"\\(?:(sub){0,3})section", line):  # section header
            ...
            sect = {"sname": "begin_doc", "level": 0, "line": i}
            struct["b"] = sect
...
def parse_environment(line, ftex_in, ftex_out, fjunk, nequ, nfig, ntab):
    ...
    if re.match(r"\\begin{equation", temp[0]):
        ...
        if len(temp) < 10:
            for e in temp:
                print("\t", e[:-1])  # skip newlines with [:-1]
        else:
            for e in temp[:9]:
                print("\t", e[:-1])
        ieq = (
            input("is this an [e]quation, [f]igure, [t]able, ite[m]ized list, or [n]one of the above?") or "n"
    ...
    if ieq.lower() == "e":  # if it does, parse it like one
        ...
        for k in range(0, len(temp)):  # this only matters if someone puts their equations in tables
            if (
                temp[k].startswith(r"\toprule")
                or temp[k].startswith(r"\endhead")
                or temp[k].startswith(r"\bottomrule")
                or temp[k].startswith(r"\end{")
            ...
            for ireplace in range(nrefs):
                ...
                if fig_num != 999:
                    ...
                    if word in plurals:  # mark plurals because we won't have caught the second number
                        ...
                        word_out = word
                    to_write = (
                        line_start + word_out + "\\ref{%s%i}" % (ref_names[iw], fig_num) + line_end
...
def _parse_paren(paren, pretext, bibkeys):
    ...
    for l in range(len(yr_inds) - 1):
        ...
        bits = [word.translate({ord(k): None for k in [".", ",", "&", "\\", ";"]}) for word in bits]
