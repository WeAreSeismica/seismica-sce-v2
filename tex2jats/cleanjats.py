...
<article-title>{}</article-title>
        ...
        if date is not None:
                ...
                """<pub-date publication-format="print" date-type="pub" iso-8601-date="{}">
    ...
    """.format(
                            orcid[i], surname[i], givenname[i], firstaffil[i] + ", " + otheraffil[i]
                        )
                    )
        else:
    ...
    """.format(
                            "", surname[i], givenname[i], firstaffil[i] + ", " + otheraffil[i], corres.split(" ")[-1]
                        )
                    )
                else:
                    fi.write(
    ...
    """.format(
                            "", surname[i], givenname[i], firstaffil[i] + ", " + otheraffil[i]
                        )
                    )
...
""".format(
                hand_editor_surname, hand_editor_givenname
            )
        )
...
""".format(
                prod_editor_surname, prod_editor_givenname
            )
        )
...
""".format(
                copyed_surname, copyed_givenname
            )
        )
...
""".format(
                        reviewer_surname[i], reviewer_givenname[i]
                    )
                )
...
""".format(
                        translator_surname[i], translator_givenname[i]
                    )
                )
...
<!-- !!! without the potentials "table" tags (<table> and <\table>) -->
    ...
    """.format(
                    labels[i], captions[i], "tab" + str(i + 1) + ".tex"
                )
            )
